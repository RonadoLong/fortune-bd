...
DOCKER_IMAGE=exchange-srv

echo ${APP_VERSION}
docker login -u admin -p QQabc123++ ${HARBOR_HOST}
docker build -t ${HARBOR_ADDR}/${DOCKER_IMAGE}:${APP_VERSION} -f ./service/exchange-srv/deploy/Dockerfile .
...
# echo '{ "insecure-registries":["192.168.3.30:8086"]}' > /etc/docker/daemon.json
