		...
		if bytes.Contains(msg, []byte("executionReport")) {
			...
			if _, ok := data["e"]; ok {
				if price, ok := data["p"]; ok {
					order.Price = global.StringToFloat64(price.(string))
				}
				if amount, ok := data["q"]; ok {
					order.Amount = global.StringToFloat64(amount.(string))
				}
				if avgPrice, ok := data["p"]; ok {
					order.AvgPrice = global.StringToFloat64(avgPrice.(string))
				}
				if dealAmount, ok := data["z"]; ok {
					order.DealAmount = global.StringToFloat64(dealAmount.(string))
				}
				if Fee, ok := data["n"]; ok {
					order.Fee = global.StringToFloat64(Fee.(string))
				}
				if Cid, ok := data["C"]; ok {
